  ...
  const filtered = useMemo(() => {
    ...
    return SECTIONS.map(section => {
      ...
      const topics = section.topics.filter(t =>
        !q || t.n.toLowerCase().includes(q) || t.desc.toLowerCase().includes(q) || t.code.toLowerCase().includes(q)
      );
      ...
      <div style={{ borderBottom: "1px solid #21262d", background: "#0d1117", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 920, margin: "0 auto", padding: "32px 20px 26px", position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: "#58a6ff", letterSpacing: 3, textTransform: "uppercase", fontWeight: 700, marginLeft: 6 }}>java.util.concurrent</span>
          </div>

          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 30, fontWeight: 800, color: "#f0f6fc", margin: "0 0 6px", letterSpacing: -1 }}>
            Java <span style={{ color: "#58a6ff" }}>Multithreading</span>
          </h1>
          <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: "#8b949e", margin: "0 0 18px" }}>
            {SECTIONS.length} categories · {totalTopics} topics — Thread basics to Virtual Threads
          </p>
        </div>
      </div>
                ...
                return (
                  <div key={topic.n}
                    ...
                    <div onClick={() => toggle(section.cat, topic.n)}
                      ...
                      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, fontWeight: 500, color: "#f0f6fc", flex: 1, lineHeight: 1.5 }}>
                        {topic.n}
                      </span>
                      <span style={{ color: section.color, fontSize: 12, flexShrink: 0, fontWeight: 600 }}>{open ? "▲" : "▼"}</span>
                    </div>

                    {/* Expanded */}
                    {open && (
                      <div style={{ padding: "0 16px 16px 34px", borderTop: "1px solid #21262d" }}>
                        ...
                        <div style={{ background: "#0d1117", border: "1px solid #30363d", borderRadius: 6, overflow: "auto" }}>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 14px", borderBottom: "1px solid #21262d", background: "#161b22" }}>
                            ...
                            <button onClick={() => copyCode(topic.code, copyKey)}
