    ...
    <div style={{ 
      ...
      <div style={{ borderBottom: "1px solid #21262d", background: "#0d1117", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 920, margin: "0 auto", padding: "32px 20px 26px", position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: "#58a6ff", letterSpacing: 3, textTransform: "uppercase", fontWeight: 700, marginLeft: 6 }}>Design Patterns</span>
          </div>

          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 30, fontWeight: 800, color: "#f0f6fc", margin: "0 0 6px", letterSpacing: -1 }}>
            Java <span style={{ color: "#58a6ff" }}>Design Patterns</span>
          </h1>
          <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: "#8b949e", margin: "0 0 18px" }}>
            {SECTIONS.length} categories · {totalTopics} patterns — Creational, Structural, Behavioral & Architectural
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ position: "relative", marginBottom: 18 }}>
          <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#58a6ff", fontSize: 14 }}>⌕</span>
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search patterns, intent, use cases, code..."
            style={{
              width: "100%", boxSizing: "border-box", padding: "10px 14px 10px 36px",
              background: "#161b22", border: "1px solid #30363d", borderRadius: 8,
              color: "#c9d1d9", fontFamily: "'IBM Plex Mono', monospace", fontSize: 13,
              outline: "none"
            }}
          />
          {search && (
            <button onClick={() => setSearch("")} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "#8b949e", cursor: "pointer", fontSize: 16 }}>×</button>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
          {categories.map(cat => {
            ...
            return (
              ...
              >
                {sec && <span style={{ marginRight: 4 }}>{sec.icon}</span>}
                {cat}
              </button>
            );
          })}
        ...
        {/* Results */}
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", color: "#8b949e", padding: "3rem", fontFamily: "'IBM Plex Sans', sans-serif" }}>
        ...
        {filtered.map(section => (
          <div key={section.cat} style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, paddingBottom: 8, borderBottom: "1px solid #21262d" }}>
              ...
              <span style={{ fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 14, fontWeight: 600, color: "#f0f6fc" }}>{section.cat}</span>
              <span style={{ marginLeft: "auto", background: "#161b22", border: "1px solid #30363d", borderRadius: 10, padding: "1px 8px", fontSize: 11, color: "#8b949e", fontFamily: "'IBM Plex Sans', sans-serif" }}>
            ...
            {section.topics.map(topic => {
              ...
              return (
                <div key={topic.n} style={{ marginBottom: 8, background: "#161b22", border: "1px solid #21262d", borderRadius: 8, overflow: "hidden" }}>
                  {/* Row header */}
                  <div
                    onClick={() => toggle(section.cat, topic.n)}
                  ...
                  >
                    <span style={{ color: "#58a6ff", fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, fontWeight: 500, flexShrink: 0, paddingTop: 1 }}>
                      {topic.n}
                      {topic.star && <span title="Most asked in interviews" style={{ color: "#f0c419", marginLeft: 6 }}>★</span>}
                    </span>
                    {!open && (
                      <span style={{ fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 12, color: "#8b949e", lineHeight: 1.5, flex: 1 }}>
                        {topic.use.length > 90 ? topic.use.slice(0, 90) + "…" : topic.use}
                      </span>
                    )}
                    <span style={{ marginLeft: "auto", color: "#58a6ff", fontSize: 14, flexShrink: 0 }}>{open ? "▲" : "▼"}</span>
                  </div>

                  {/* Expanded detail */}
                  {open && (
                    <div style={{ padding: "0 16px 16px 16px" }}>
                      <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 13, color: "#8b949e", lineHeight: 1.7, margin: "0 0 14px 0" }}>
                        <div style={{ marginBottom: 8 }}>
                          <span style={{ color: "#58a6ff", fontWeight: 600, fontSize: 11, letterSpacing: 1 }}>INTENT</span>
                          <span style={{ marginLeft: 8 }}>{topic.intent}</span>
                        </div>
                        {topic.theory && (
                          <div style={{ marginBottom: 8 }}>
                            <span style={{ color: "#58a6ff", fontWeight: 600, fontSize: 11, letterSpacing: 1 }}>WHY THIS CATEGORY</span>
                            <span style={{ marginLeft: 8 }}>{topic.theory}</span>
                          </div>
                        )}
                        <div style={{ marginBottom: topic.antipattern || topic.related ? 8 : 0 }}>
                          <span style={{ color: "#58a6ff", fontWeight: 600, fontSize: 11, letterSpacing: 1 }}>USE</span>
                          <span style={{ marginLeft: 8 }}>{topic.use}</span>
                        </div>
                        {topic.antipattern && (
                          <div style={{ marginBottom: 8 }}>
                            <span style={{ color: "#f0c419", fontWeight: 600, fontSize: 11, letterSpacing: 1 }}>ANTI-PATTERN ANGLE</span>
                            <span style={{ marginLeft: 8 }}>{topic.antipattern}</span>
                          </div>
                        )}
                        {topic.related && (
                          <div>
                            <span style={{ color: "#58a6ff", fontWeight: 600, fontSize: 11, letterSpacing: 1 }}>RELATED CONCEPTS</span>
                            <span style={{ marginLeft: 8 }}>{topic.related}</span>
                          </div>
                        )}
                      </div>
                      {/* Code block */}
                      <div style={{ background: "#0d1117", border: "1px solid #30363d", borderRadius: 6, overflow: "auto" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 14px", borderBottom: "1px solid #21262d" }}>
                          <span style={{ fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 11, color: "#8b949e", textTransform: "uppercase", letterSpacing: 1 }}>Java</span>
                          <button
                            onClick={() => copy(topic.code, copyKey)}
                            style={{ background: "none", border: "1px solid #30363d", borderRadius: 4, padding: "2px 8px", color: "#8b949e", cursor: "pointer", fontSize: 11, fontFamily: "'IBM Plex Sans', sans-serif" }}
                          >
                            {copied === copyKey ? "✓ copied!" : "copy"}
                          </button>
                        </div>
                        <pre style={{ margin: 0, padding: "14px", fontSize: 12, lineHeight: 1.65, color: "#e6edf3", fontFamily: "'IBM Plex Mono', monospace", whiteSpace: "pre", overflowX: "auto" }}>
                          {topic.code}
                        </pre>
                      </div>
                    </div>
